...
export default function ConfigMgmtEditPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (supaErr) {
      setError('수정에 실패했습니다: ' + supaErr.message);
      setLoading(false);
      return;
    }
  ...
  if (fetching) return <div style={{ textAlign: 'center', padding: '4rem', color: '#6b7280' }}>불러오는 중...</div>;

  return (
    <div className={styles.page}>
      <div className={styles.card}>
        <div className={styles.cardHeader}>
          <h1 className={styles.heading}>형상관리 항목 수정</h1>
          <p className={styles.sub}>형상관리 항목 정보를 수정합니다</p>
        </div>
